import React from 'react'

function CardLocations() {
  return (
    <section className='px-12 sm:px-24 py-1 flex flex-wrap gap-6 mb-24'>
          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-5.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-6.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-7.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-8.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-1.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-2.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343]'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-3.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-4.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          {/* New Sections */}
          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-9.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-10.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-11.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-12.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-13.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-14.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343]'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-15.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>

          <div className="card-container p-4 basis-full md:basis-[48%] xl:basis-[23%]">
            <div>
              <img src="img/img-16.jpg" alt="" className='w-full' />  
              <img src="img/love" alt="" />
            </div>
            <div className='flex justify-between py-4'>
              <p className='text-xs text-[#434343]'>Desert king</p>
              <p className='text-xs text-[#434343] font-bold'>1MBT per night</p>
            </div>

            <div className='flex justify-between pb-4'>
              <p className='text-xs text-[#434343]'>2345km away</p>
              <p className='text-xs text-[#434343] text-right'>available for 2weeks stay</p>
            </div>

            <div><img src="img/star.svg" alt="" /></div>
          </div>
        </section>
  )
}

export default CardLocations